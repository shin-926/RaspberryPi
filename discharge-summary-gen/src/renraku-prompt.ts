...
【マオカ病院の運用前提】
- 病棟構成: **一般病棟（急性期寄り）+ 療養病棟（長期ケア）**
- 一般病棟で急性期治療を行い、落ち着いたら療養病棟へ転棟するケースあり
- 入院時JCSは両病棟とも基本0（明確な記述がある場合のみ1桁範囲で昇格、稀に2桁以上）
- ほぼ全症例で入院目的=4（その他の加療）
- **手術は一切実施しないため手術関連項目は対象外**（骨折・整形系も保存療法）

【主傷病・契機病名・医療資源病名の判定指針 — 極めて重要】

DPC様式1には3つの病名概念があり、定義が**別物**である:

■ trigger_disease（入院契機病名 A006020）= **入院することになった直接のきっかけ**
- 「自宅で転倒→骨折→入院」「肺炎発症→入院」「脳梗塞発症→搬送」等の事象の中心疾患
- **現在管理中の併存疾患（認知症・高血圧・糖尿病・心不全等）を契機病名にしてはならない**
- 最も参考になるのは患者プロフィールの「現病歴」セクション
- 外傷系（ICD-10のS/T系）、急性疾患（脳卒中I63/I61、肺炎J18等）が契機になることが多い

■ main_disease（主傷病 A006010）= **退院時の総合評価で最も主たる疾患**
- 入院期間全体で「治療・ケアの中心」となった疾患
- 入院後に重篤な疾患（誤嚥性肺炎・脳梗塞等）を新規発症して治療の中心が移った場合、主傷病はその新規発症疾患になりうる
- 通常は契機病名と同じだが、入院中の急変で別疾患に置き換わるケースあり

■ resource_disease（医療資源を最も投入した病名 A006030）= **資源量で判定**
- 在院中、薬剤・処置・看護で最も資源を投入した疾患
- 手術ありなら手術対象疾患になりやすい
- 終末期に集中治療した疾患（例: 誤嚥性肺炎での抗生剤+酸素+点滴）が選ばれることもある

【マオカ病院の3病名運用指針】
- 一般病棟: 急性期治療目的の入院(骨折保存療法・脳卒中後管理・肺炎治療等)
- 療養病棟: 長期ケア目的の入院
- **両病棟とも、原則として trigger = main = resource の3つは一致**
- 例外を意識すべきケース:
  1. 入院中に**新たに重篤な疾患を発症**して治療の中心が変わった → main/resource が新規疾患に変わりうる
  2. 入院契機の疾患が**短期で治癒**して、別の合併症の治療が長期化した → main が変わりうる
  3. がん患者で**緩和治療**が長引いた → resource ががんになる
- 3つは独立に判定するが、上記例外がなければ素直に一致させる（一貫性も大事）
...
{
  "main_disease": {
    "uuid": "string（病名候補リストの uuid を選択。退院時の主たる疾患）",
...
export function validateLlmDecisions(
  ...
  if (decisions.main_disease?.uuid && !validDiseaseUuids.has(decisions.main_disease.uuid)) {
    warnings.push(`main_disease.uuid (${decisions.main_disease.uuid}) が候補リストにない`);
  }
